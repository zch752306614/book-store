<template>
  <view class="page-index" :style="paddingTop">
    ...
    <view v-if="recodeList.length > 0" class="index-read">
      <view class="index-read-content">
        <view class="read-top">
          <view class="top-left">
            <text class="top-title">已阅</text>
            <u-badge numberType="overflow" shape="horn" bgColor="#000" max="99" :value="recodeList.length"></u-badge>
          </view>
          <text v-if="idList.length > 6" class="top-more" @click="toRecodeMore">查看更多</text>
        </view>
        <view class="read-list">
          <view v-for="(item, index) in recodeList" :key="index" class="list-one" @click="toBookread(item)">
  ...
  methods: {
    ...
    getBookRecodeList () {
      try {
        ...
        if (idList) {
          this.idList = JSON.parse(idList)
        }
      } catch (e) { }
      if (this.idList.length > 0) {
        this.$Api.default.getBookList({ idList: this.idList, ...this.pageObj }, false, false).then(res => {
          this.recodeList = res.data.rows || []
        })
      } else {
        this.recodeList = []
      }
